...
import { debugSessionLog, getNavigationDebugInfo } from "@/lib/debugSession";
...
function MixerPage() {
  ...
  const profileAvatarKey = useMemo(
    () => `${user?.id ?? "guest"}:${getUserAvatarUrl(user) ?? "none"}`,
    [user],
  );
  const {
    tracks,
  ...
  const [appPhase, setAppPhase] = useState<AppPhase>("unauthenticated");
  ...
  const appInitGenerationRef = useRef(0);
  const showBootScreen = isAppInitializing || appPhase === "projectLoading";
  ...
  const performProjectSave = useCallback(
    ...
    [currentProjectId, isHydrating, isProjectEditable, persistedProjectConfig]
  );
  ...
  useEffect(() => {
    bootLog("mixer:mounted", pathname);
    return () => {
      bootLog("mixer:unmounted");
    };
  }, [pathname]);

  useEffect(() => {
    markBootStarted();
    bootLog("page:client-mounted", pathname);
  }, [pathname]);
  ...
  useEffect(() => {
    bootLog(
      ...
      JSON.stringify({
        ...
        projectIdFromPath: projectIdFromPath ?? "none",
      }),
    );
  }, [isAuthReady, isAppInitializing, appPhase, sessionUserId, projectIdFromPath]);
  ...
  }, [appPhase, isAuthReady, isAppInitializing, sessionUserId, showBootScreen]);

  useEffect(() => {
    if (!isAuthReady) {
      bootLog("init:wait-auth-ready");
      const fallbackId = window.setTimeout(() => {
        bootLog("init:auth-ready-fallback", "proceeding as guest");
    ...
    const generation = ++appInitGenerationRef.current;
    let cancelled = false;
    bootLog("init:start", `generation=${generation} path=${pathname} session=${sessionUserId ? "yes" : "no"}`);

    const isCurrentInit = () => !cancelled && generation === appInitGenerationRef.current;

    const initialize = async () => {
      const showInitOverlay = Boolean(projectIdFromPath || sessionUserId);
      if (showInitOverlay && isCurrentInit()) {
        setIsAppInitializing(true);
      }
      bootLog("init:running", `generation=${generation}`);
      try {
        if (!supabase) {
          bootLog("init:no-supabase");
        ...
        if (projectIdFromPath) {
          bootLog("init:load-project", projectIdFromPath);
          loadingProjectRef.current = true;
          if (isCurrentInit()) setAppPhase("projectLoading");
          const { data, error } = await supabase
            .from("projects")
            .select("id,title,config,updated_at,user_id")
            .eq("id", projectIdFromPath)
            .single();
          bootLog("init:project-fetched", error ? `error=${error.message}` : `found=${Boolean(data)}`);
          if (!isCurrentInit()) {
            bootLog("init:aborted", "after project fetch");
            return;
          }
          if (error || !data) {
            ...
            if (sessionUserId) {
              bootLog("init:project-missing", "redirect authenticated user to dashboard");
              router.replace("/");
              const rows = await refreshProjects();
              if (!isCurrentInit()) return;
              setAppPhase(rows.length > 0 ? "dashboard" : "onboarding");
            } else {
              bootLog("init:project-missing", "redirect guest to landing");
              router.replace("/");
              setAppPhase("unauthenticated");
            }
            return;
          ...
          try {
            ...
            bootLog("init:resolve-audio-urls", `${hydrated.tracks.length} tracks`);
            const hydratedTracks = await resolveTrackPlaybackUrls(hydrated.tracks);
            bootLog("init:audio-urls-ready");
            if (!isCurrentInit()) {
              bootLog("init:aborted", "after audio url resolve");
              return;
            }
            replaceProjectState({
            ...
            setCurrentProjectId(data.id);
            setCurrentProjectOwnerId(data.user_id ?? null);
            ...
            bootLog("init:done", "phase=workspace");
          } finally {
            setIsHydrating(false);
          }
          return;
        ...
        if (!sessionUserId) {
          ...
          setSaveState("idle");
          bootLog("init:done", "phase=unauthenticated");
          return;
        }

        bootLog("init:refresh-projects");
        const rows = await refreshProjects();
        bootLog("init:projects-ready", `count=${rows.length}`);
        if (!isCurrentInit()) {
          bootLog("init:aborted", "after refreshProjects");
          return;
        }
        const nextPhase = rows.length > 0 ? "dashboard" : "onboarding";
        setAppPhase(nextPhase);
        bootLog("init:done", `phase=${nextPhase}`);
      } catch (cause) {
        const message = cause instanceof Error ? cause.message : "unknown error";
        bootLog("init:error", message);
        ...
        if (isCurrentInit()) {
          setIsAppInitializing(false);
          bootLog("init:finally", "isAppInitializing=false");
        } else {
          bootLog("init:finally-skipped", `generation=${generation} cancelled or stale`);
        }
      }
    };
    ...
    return () => {
      cancelled = true;
      bootLog("init:cleanup", `generation=${generation}`);
    };
  }, [
    isAuthReady,
    pathname,
  ...
  useEffect(() => {
    ...
    const returnTo = sessionStorage.getItem("foam_auth_return_to");
    if (!returnTo) return;
    try {
      const returnUrl = new URL(returnTo, window.location.origin);
      const onHome = window.location.pathname === "/";
      if (onHome && returnUrl.pathname !== "/") {
        sessionStorage.removeItem("foam_auth_return_to");
  ...
  }, [currentProjectConfig, currentProjectId, isHydrating, isProjectEditable, performProjectSave]);
  ...
  const loadProject = useCallback(
    async (project: ProjectRow) => {
      ...
      try {
        ...
        setCurrentProjectOwnerId(project.user_id ?? sessionUserId);
    ...
    [replaceProjectState, resolveTrackPlaybackUrls, router, sessionUserId]
  );
  ...
  const handleRenameProjectFromDashboard = useCallback(
    async (projectId: string, nextTitle: string) => {
      ...
      if (error) {
        ...
        appToast.success("Project renamed");
      }
    },
  ...
  const handlePreviewLogin = () => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem("foam_auth_return_to", window.location.href);
    }
    router.push("/");
  };

  const handlePreviewOAuthLogin = async (provider: "google" | "twitter") => {
  ...
  const sidebarContent = (
    <>
      ...
      <section className={styles.section}>
        ...
        <div className={styles.previewPlayable}>
          <PlayerBar
            isPlaying={isPlaying}
  ...
  if (showBootScreen) {
    const bootMessage =
      appPhase === "projectLoading" ? "Loading project..." : "Loading workspace...";
  ...
  const workspaceHeader = session ? (
    <header className={styles.workspaceHeader}>
      ...
      <div className={styles.profileWrap} ref={profileWrapRef}>
        {workspaceActive && currentProjectId ? (
          <button
            type="button"
              ...
              isDemoScene ? (
                ...
                <>
                  <input
                    ...
                    disabled={profileBusy || projectTitleBusy}
                  />
                  <button
                    type="button"
                    className={styles.profileAction}
      ...
      {workspaceActive && isReadOnlyPreview ? (
        <div className={styles.previewDock}>
          ...
          {sessionUserId ? (
            ...
            <div className={styles.previewLoginGroup}>
              <button
                type="button"
                className={styles.previewPrimaryBtn}
              ...
              <button
                type="button"
                className={styles.previewOAuthBtn}
                aria-label="Continue with Google"
                onClick={() => void handlePreviewOAuthLogin("google")}
              >
                <Image
                  src="/google_logo.svg"
                  alt=""
                  width={22}
                  height={22}
                  className={styles.previewOAuthLogoGoogle}
                  aria-hidden
                />
              </button>
              <button
                ...
                onClick={() => void handlePreviewOAuthLogin("twitter")}
              >
                <Image src="/X_logo.svg" alt="" width={22} height={22} aria-hidden />
              </button>
            </div>
          )}
        </div>
