...
export function Landing({ onStartClean, onStartDemo }: LandingProps) {
  ...
  const handleContinueWithEmail = () => {
    ...
    const providers = new Set((hint?.providers ?? []).map((provider) => provider.toLowerCase()));
    ...
    await withBusy(async () => {
      ...
      if (signUpError) {
        if (/already registered|already been registered/i.test(signUpError.message)) {
          ...
          transitionTo("login");
          throw new Error("Account already exists. Enter your password to sign in.");
        }
        throw signUpError;
      }
      rememberPasswordLogin(normalizedEmail);
  ...
  return (
    <div className={styles.overlay}>
      <section className={styles.panel} style={{ viewTransitionName: "auth-card" }}>
        {(isBusy || isLoading) ? <span className={styles.spinner} aria-hidden /> : null}
        <h1 className={styles.logo} style={{ viewTransitionName: "auth-title" }}>foam</h1>
        {step !== "setup" ? (
          <p className={styles.description}>
            Sign in to your workspace and continue building spatial audio scenes.
          </p>
        ) : (
          <p className={styles.description}>
            A professional-grade spatial audio simulator for sound engineers and educators.
            Calculate acoustic distribution, critical distance, and occlusion with engineering precision.
          </p>
        )}
        ...
        {step === "entry" ? (
          <div className={styles.authBody}>
            <div className={styles.oauthGrid}>
              <button type="button" className={styles.oauthBtn} disabled={controlsDisabled} onClick={() => void handleOAuth("google")}>Google</button>
            </div>
            ...
            >
              ...
              />
              <button className={styles.startClean} style={{ viewTransitionName: "auth-submit-btn" }} type="submit" disabled={controlsDisabled}>Next</button>
            </form>
            {emailHintMessage ? <p className={styles.inlineError}>{emailHintMessage}</p> : null}
          </div>
        ) : null}
        ...
        {step === "login" ? (
          ...
          >
            ...
            <input className={styles.input} type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="password" disabled={controlsDisabled} />
            <button className={styles.startClean} style={{ viewTransitionName: "auth-submit-btn" }} type="submit" disabled={controlsDisabled}>Sign In</button>
            <button type="button" className={styles.linkBtn} onClick={() => void handleForgotPassword()} disabled={controlsDisabled}>Forgot Password?</button>
          </form>
        ) : null}
        ...
        {step === "register" ? (
          ...
          >
            ...
            <input
              ...
              disabled={controlsDisabled}
            />
            <input className={styles.input} type="password" autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="password" disabled={controlsDisabled} />
            <input className={styles.input} type="password" autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="confirm password" disabled={controlsDisabled} />
            <button className={styles.startClean} style={{ viewTransitionName: "auth-submit-btn" }} type="submit" disabled={controlsDisabled}>Create Account</button>
            <button type="button" className={styles.linkBtn} onClick={() => transitionTo("login")} disabled={controlsDisabled}>Already have an account? Log in</button>
          </form>
        ) : null}
        ...
        {step === "confirm" ? (
          <div className={styles.noticeBlock}>
            ...
            <button type="button" className={styles.startDemo} onClick={() => transitionTo("login")} disabled={controlsDisabled}>Back to sign in</button>
          </div>
        ) : null}
